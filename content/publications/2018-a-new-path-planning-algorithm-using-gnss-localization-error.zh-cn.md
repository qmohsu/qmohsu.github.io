---
title: "A New Path Planning Algorithm Using GNSS Localization Error Map for UAV in Urban Area"
slug: "2018-a-new-path-planning-algorithm-using-gnss-localization-error"
authors:
  - "Zhang G."
  - "Hsu L.T."
year: 2018
venue: "Journal of Intelligent & Robotic Systems"
type: "journal"
cv_number: 120
featured: false
quartile: "Q2"
doi: "10.1007/s10846-018-0894-5"
pdf: ""
code: ""
data: ""
themes:
  - "integrity-localization"
  - "seamless-pnt-embodied"
tags: []
---

<aside class="translation-stub">本论文以英文发表。完整书目详情、DOI / PDF 链接请见 <a href="/en/publications/2018-a-new-path-planning-algorithm-using-gnss-localization-error/">英文页面</a>。</aside>
